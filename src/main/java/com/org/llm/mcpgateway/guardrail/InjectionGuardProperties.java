 ...
 * Applied to incoming tool call arguments to detect attempts to inject instructions
 * through tool inputs (indirect injection via MCP tool calls).
 *
 * <p>Patterns are loaded from {@code app.security.injection-guard.patterns} in application.yaml.
...
public class InjectionGuardProperties {
    ...
    private String blockMessage = "Tool call rejected: input contains disallowed instructions.";

    /**
     * Regex patterns that trigger a block on tool call arguments.
     * Override via {@code app.security.injection-guard.patterns[0]=...} in application.yaml.
     */
